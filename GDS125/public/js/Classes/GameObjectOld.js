...
{	
	...
	this.drawStaticImage=function(_regX=-this.width/2,_regY=-this.height/2,_imageWidth=this.width, _imageHeight=this.height)
	{
		if(ready)
		{
			context.save()	
			...
			context.drawImage(this.img, _regX,_regY, _imageWidth, _imageHeight)
			context.restore();
		}
		return this;
	}
	...
	this.left = function() 
	{
		return {x:this.x - this.width/2 - 1 , y:this.y, world:this.world}
	}
	this.right = function() 
	{
		return {x:this.x + this.width/2 + 1 , y:this.y, world:this.world}
	...
	this.top = function() 
	{
		return {x:this.x, y:this.y - this.height/2 - 1, world:this.world}
	}
